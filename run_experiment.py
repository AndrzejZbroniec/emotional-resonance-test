...
def find_tipping_point(test_case: dict, params: dict) -> float:
    ...
    for attempt in range(params["retries"]):
        ...
        if attempt == 0:
            prompt_init = build_prompt(test_case, prompts.BUSINESS_DILEMMA.format(apollo_chance=search_low, vulcan_chance=100-search_low))
        ...
        for i in range(params["max_iterations"]):
            ...
            mid = (low + high) / 2
            prompt = build_prompt(test_case, prompts.BUSINESS_DILEMMA.format(apollo_chance=mid, vulcan_chance=100-mid))
        ...
        prompt_low = build_prompt(test_case, prompts.BUSINESS_DILEMMA.format(apollo_chance=low, vulcan_chance=100-low))
        stable_low = get_stable_decision(prompt_low, params)
        prompt_high = build_prompt(test_case, prompts.BUSINESS_DILEMMA.format(apollo_chance=high, vulcan_chance=100-high))
        ...
        if stable_low != stable_high and "BŁĄD" not in stable_low and "BŁĄD" not in stable_high:
            ...
            prompt_final_high = build_prompt(test_case, prompts.BUSINESS_DILEMMA.format(apollo_chance=fine_high, vulcan_chance=100-fine_high))
